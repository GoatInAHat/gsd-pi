...
test("parent-workspace verification defaults to the orchestration root", () => {
  const cwd = mkdtempSync(join(tmpdir(), "gsd-source-parent-workspace-"));
  tempDirs.add(cwd);
  mkdirSync(join(cwd, ".gsd"), { recursive: true });
  mkdirSync(join(cwd, "frontend"));
  mkdirSync(join(cwd, "backend"));

  const resolved = resolveVerificationRepositoryTargets(cwd, {
    workspace: {
      mode: "parent",
      repositories: {
  ...
  assert.equal(resolved.repositories[0]?.root, cwd);
