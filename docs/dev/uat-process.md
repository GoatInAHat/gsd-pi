...
4. `complete-milestone` runs only after every slice has a UAT assessment with verdict `PASS` and validation records a passing milestone verdict. During closeout, auto mode redispatches UAT-scoped non-passing verdicts for fresh evidence. If a slice still lacks a PASS verdict, closeout stops with recovery guidance to run `/gsd dispatch uat`, request a slice-specific UAT rerun, or add remediation slices with `/gsd dispatch reassess`.
...
- Milestone closeout requires each slice assessment to record `PASS`; it retries UAT-scoped non-passing verdicts before blocking with steps to rerun UAT, inspect `/gsd status`, or dispatch reassessment work.
