...
- **Model Catalog Overlay**: the versioned JSON snapshot at `~/.gsd/agent/models-catalog.json` (`{ version: 1, fetchedAt, source, models: { provider: { modelId: {...} } } }`), written by `gsd update --models` from the published catalog (`https://raw.githubusercontent.com/open-gsd/gsd-pi/main/packages/pi-ai/src/models.generated.json`). At startup the ModelRegistry layers it between the bundled catalog (`models.generated.ts`) and the user's `~/.gsd/agent/models.json`: overlay entries replace bundled entries with the same provider+model id and add new models/providers, while models.json custom providers and overrides keep highest precedence. A missing or malformed overlay is ignored and never breaks startup or clobbers user data. It refreshes models, pricing, and context windows without a full npm upgrade.
