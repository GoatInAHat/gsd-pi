...
function makeTempDir(): string {
	return mkdtempSync(join(tmpdir(), "pi-test-models-catalog-overlay-"));
}
...
test("model-registry catalog overlay: missing overlay file is a no-op", () => {
...
test("model-registry catalog overlay: adds a new model to an existing provider", () => {
	const tempDir = makeTempDir();
	try {
		...
		// Bundled anthropic models are still present.
...
test("model-registry catalog overlay: replaces a bundled entry wholesale", () => {
	const tempDir = makeTempDir();
	try {
		const bundled = getModels("anthropic" as Parameters<typeof getModels>[0]) as Model<Api>[];
...
test("model-registry catalog overlay: adds a brand-new provider", () => {
	const tempDir = makeTempDir();
	try {
		writeOverlay(tempDir, {
			"future-labs": {
...
test("model-registry catalog overlay: malformed JSON is ignored without breaking startup", () => {
	const tempDir = makeTempDir();
	try {
...
test("model-registry catalog overlay: wrong shape is ignored without breaking startup", () => {
	const tempDir = makeTempDir();
	try {
		writeFileSync(join(tempDir, "models-catalog.json"), JSON.stringify({ version: 1, models: ["nope"] }));

		const registry = makeRegistry(tempDir);
		const error = registry.getError();
		assert.ok(error, "wrong-shape overlay should surface a non-fatal error");
		assert.match(error, /models-catalog\.json/);
		assert.ok(registry.getAll().length > 0, "bundled models should still load");
...
test("model-registry catalog overlay: models.json custom entry wins over the overlay", () => {
	const tempDir = makeTempDir();
	try {
		writeOverlay(tempDir, {
			anthropic: {
...
test("model-registry catalog overlay: models.json modelOverrides apply on top of the overlay", () => {
	const tempDir = makeTempDir();
	try {
		writeOverlay(tempDir, {
			anthropic: {
